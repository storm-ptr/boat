...
inline db::variant get_blob(PGresult* res, int row, int col)
{
    ...
    if (auto mem = unique_ptr<void, PQfreemem>(PQunescapeBytea(ptr, &len)))
        return db::variant{std::in_place_type<blob>, as_bytes(mem.get()), len};
    throw std::runtime_error{concat(PQfname(res, col), " PQunescapeBytea")};
}

inline db::variant get_value(PGresult* res, int row, int col)
{
