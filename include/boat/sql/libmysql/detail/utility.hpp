...
#ifndef BOAT_SQL_LIBMYSQL_UTILITY_HPP
#define BOAT_SQL_LIBMYSQL_UTILITY_HPP
...
inline MYSQL_BIND to_bind(db::variant const& var)
{
    auto ret = MYSQL_BIND{};
    auto vis = overloaded{
        [&](db::null const&) { ret.buffer_type = MYSQL_TYPE_NULL; },
        [&](int64_t const& v) {
    ...
    unsigned long len)
{
    ...
    switch (fld.type) {
        ...
        case MYSQL_TYPE_VARCHAR:
            return db::variant{std::in_place_type<std::string>, ptr, len};
    }
...
#endif  // BOAT_SQL_LIBMYSQL_UTILITY_HPP
