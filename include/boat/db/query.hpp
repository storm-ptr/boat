...
class query : std::vector<std::variant<std::string, id, variant>> {
public:
    query(auto&&... vals)
    ...
    query& operator<<(std::convertible_to<std::string_view> auto&& sql)
    {
    ...
    query& operator<<(variant param)
    {
        push_back(std::move(param));
        return *this;
    }

    std::string text(char id_quote, std::string_view param_mark) const
    {
    ...
    std::generator<std::reference_wrapper<variant const>> params() const
    {
        for (auto& item : *this)
            if (auto* ptr = std::get_if<variant>(&item); ptr and *ptr)
